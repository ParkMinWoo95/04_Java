...
public class Setservice {
	...
	/**
	 * Hash 단어가 포함된 컬렉션은 무조건 equals(), hashCode()가 오버라이딩 되어있어야한다
	 * 
	 ...
	 * - 해당 Set에 저장된 요소를 순차 접근할 수 있는 반복자(Iterator)를 반환하는 메서드
	 * 
	 * [Iterator 인터페이스]
	 * - 반복자 기능 제공 인터페이스
	 * - <E> 반복자가 반환하는 요소의 유형
	 ...
	 * 2) E next() : 다음 요소를 반환 -> 무조건 hasNext() 이후에 사용
	 */
	public void test3() {
		...
		/* 향상된 for문
		 * - 배열, 컬렉션(List,Set)의 요소를 하나씩 추가 순차 접근
	...
	public String getLottoNumber() {
		...
		while(lotto.size() < 6) {	// 저장된 값이 6개 미만이면 반복
...
//			int num = random.nextInt(45) + 1; // 1 ~ 45 사이 난tn
//			lotto[i] = num;
//			
//			for(int x=0; x<i; x++) {
//				if(lotto[x] == num) {
