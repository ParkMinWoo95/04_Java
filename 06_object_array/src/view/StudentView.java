...
public class StudentView {
	...
	private void allStudent() {
		...
		// StudentService 객체에서 모든 학생을 저장한 객체 배열을 얻어와 화면에 출력
		...
		for(StudentDTO std : students) {
			...
			// 참조 변수에 저장된 주소가 출려되면 이상하니까 컴파일러가 자동으로
			// 참조변수.toString()을 호출하도록 변경
	...
	/* 학생 추가 view 메서드
	 * <pre>
	 * 	학번, 이름, 성별을 입력받아 StudentDTO 객체를 만들어서
	 ...
	 * - 인덱스를 입력받아 Student.Service.selectIndex()로 전달
	...
	private void selectIndex() {
		...
		// Student.Service.selectIndex()로 전달
	 ...
	 *  
	 *  - 입력 받은 인덱스에 학생이 있을 경우 HTML, CSS, JS, Java 점수를 순서대로 입력 받은 후
	 ...
	 *  2) 참조하는 학생이 없을 경우 : "해당 인덱스에 학생이 존재하지 않습니다" 출력
	 * 
	 * - 인덱스가 정상 입력 되었다면 해당 인덱스 번째 학생(StudentDTO)을 반환 받기
	 * - 합계, 평균 구하기
	 * - 아래와 같은 형식으로 출력하기
	 * ----------------------------------------------
	 * [20250001] 짱구(남)
