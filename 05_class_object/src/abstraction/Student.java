...
public class Student {
	...
	// private 접근 제한다 - 외부 직접 접근을 제한(캡슐화 원칙)
	...
	/* 기본 생성자
	 * - 매개 변수가 없는 생성자
	 * - 클래스 내에 생성자가 1개도 없으면 컴파일러가 자동으로 생성	
	 * -> public 클래스명(){}
	 */
	public Student() {
		System.out.println("--Student 객체가 기본생성자로 생성되었습니다--");
		
		// 객체가 heap 영역에 생성될 때 필드 값이 모두 JVM 초기값으로 세팅이 진행됨
		// - heap 영역은 비어있는 상태로 존재 불가
		// - 정수형 : 0, 실수형 : 0.0
		//	 논리형 : false, 문자형 : '\u0000'
		//	 참조형 : null (참조하는 객체가 없다, 저장된 주소가 없다)
		
		// 생성자를 이용해서 객체가 생서 될 떄 필드값을 원하는 값으로 초기화 할 수 있다
	 ...
	 * - 같은 이름의 메서드는 원래 하나만 존재할 수 있지만 매개변수(순서, 타입, 개수)를
	 * 	 다르게하여 같은 이름의 메서드라도 구분할 수 있게 된다
	 */
	...
	// setter : 외부에서 전달 받은 값을 객체의 필드에 세팅하는 메서드
