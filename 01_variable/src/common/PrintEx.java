...
public class PrintEx {
	...
	public static void main(String[] args) {
		 ...
		 * - 문자열 템플릿 중간에 "%"로 시작하는 패턴을 입력하여 원하는 변수/값을 대입할 수 있다
		 * 
		 * [패턴 종류]
		 * - %d (Demical, 10진 정수) : 정수(byte, short, int, long)
		 * - %c (Character, 문자) : 문자(char)
		 * - %s (String, 문자열) : 문자열(String)
		 * - %b (boolean, 논리형) : 논리형(boolean)
		 * - %f (float, 실수) : 실수형(flaot)
		 * 
		 * - 양의 정수 : 정수 크기만큼 칸 확보 + 오른쪽 정렬
		 * ex) %4d, 3 대입 -> ㅁㅁㅁ3 (ㅁ은 빈칸 표시)
		 * - 음의 정수 : 정수 크기만큼 칸 확보 + 왼쪽 정렬
		 * ex) %-4d, 3 대입 -> 3ㅁㅁㅁ
		 * 
		 * - 소수점 : 소수점 아래 몇째 자리까지 표시할지 지정(지정된 자리 아래에서 반올림 처리)
		 * ex) %.1f, 3.15 대입 -> 3.2
		 */
		...
		// println() 버전
		System.out.println(name + "는 " + age + "세 " + gender + "아로 키는 " + height
											+ "cm 이며, 자바 공부 여부 : " + javaStudy);
		// System.out.printf("%s는 %d세 %c아로 키는 %.1fcm 이며, 자바 공부 여부 : %b", name, age, height, gender, javaStudy);
