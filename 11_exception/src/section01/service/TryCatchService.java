...
public class TryCatchService {
	 ...
	 * (if, try-catch, throws)
	 * 
	 * Exception Handing(예외 처리) : 발생된 예외를 처리하는 구문(try-catch, throws) 
	 */
	
	/** try-catch 작성법
	 * <pre>
	 * try {예외가 발생할 것 같은 구문}4
	 * - 예외가 발생할 가능3성이 있는 코드를 시도하는 구문
	 * 
	 * catch(예외 참조 변수){}
	...
	public void test2() {
		...
		try {
			...
			// 		위에서 부터 순대로 탐색 -> 알맞은 catch문에 예외 처리 수행
...
//		}
	...
	/* finally{}
	 * - 뜻 : 마침내, 마지막으로
	 * - try 구문에서 예외 발생 여부 관계없이 마지막에 무조건 실행되어야 하는 코드를 작성하는 구문
	 * - 중간에 프로그램이 중료되거나 메서드 리턴되어도 무조건 실행
