...
public class Korean {
	...
	/* static(정적인, 고정된)
	 * - 고정된 영역(static 메모리 영역)에 모든 객체가 공유할 멤버(필드/메서드)를 생성해서 공유
	 * - static이 붙는 멤버는 [프로그램 실행 시] static 영역에 할당되고 [프로그램 종료 시] 사라지게 된다
	 */
	 ...
	 * - 생성 시기 : 객체 생성 시
	 * - 소멸 시기 : 객체 소멸 시
	 * 	 (어떤 변수도 객체를 참조하지 않을 경우 Heap 메모리 영역의
	 * 		GC(Garbage Collection)가 메모리 반환)
	 */
	
	// 인스턴스 초기화 블럭 : 인스턴수 변수 초기화 목적
	{
	...
	// 기본 생성자
	public Korean() {}	// 클래스에 생성자가 없을 경우 컴파일러가 자동으로 추가함
