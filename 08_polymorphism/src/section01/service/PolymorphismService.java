...
public class PolymorphismService {
	 ...
	 * 
	 * 매개 변수(Parameter) : 메서드 호출 시 전달된 값을 저장 하는 변수
	 */
	
	/* 참조변수 instanceof 클래스명
	 * 
	 * - "참조변수"가 참조하는 객체가 "클래스명"로 만들어진 객체가 맞을 경우 true, 아니면 false
	 * - 상속 관계
	 */
	...
	public void test1() {
		...
		Child child = new Child("이", 600, "소나타");
	...
	/* [반환형의 다형성]
	 * - 메서드의 반환형을 부모 타입을 설정하여 return되는 자식 객체의 주소를 참조할 수 있게 함
	 * - 다형성의 업캐스팅 적용
	 */
	...
	public Object getInstance(int num) {
		...
		// if, for, while의 {} 내부 코드가 한 줄인 경우 {} 생략 가능
	...
	public void test3() {
		...
		// (정적)section01.dto.Parent.toString ()
		// (동적)section01.dto.Child.toString () <- 변함
	...
	public void test4() {
		...
		// 동적바인딩
		// - 실행 중 메서드 호출부 연결이 실제 참조하는 객체의 오버라이딩된 메서드로 연결
		 ...
		 * - 코드 수행 중 지정된 지점(breakpoint)에서 실행을 멈추고 해당 시점에 존재하는 변수/필드 값을 확인하는 모드
		 */
	...
	public void test6(Object obj) {
		 ...
		 * - 참조 변수의 강제 형변환(다운 캐스팅) 시 참조하는 객체가 변환하려는 타입이
		 * 	 아니거나 상속 관계도 아니면 형변환 불가(ClassCastException) 발생
		 */
		
		// 참조하는 객체가 String 타입일 경우에만 형변환
