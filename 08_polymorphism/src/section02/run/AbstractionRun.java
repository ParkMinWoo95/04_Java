...
public class AbstractionRun {
	public static void main(String[] args) {
		...
		// Animal의 eat() 메서드는 추상 메서드로 원래는 호출이 불가능해야 하지만
		// 참조하는 객체가 eat()를 구현한 자식 타입이므로 컴파일 시(정적바인딩) 오류가 발생하지 않는다
	...
	public static void printAnimal(Animal animal) {
		...
		// 매개 변수 Animal(추상 클래스)
		// - 부모 타입의 참조 변수로 사용(다형성 - 업 캐스팅)
		// - Animal을 상속 받은 클래스는 반드시 상속받아 재정의한 eat(), move(), sleep() 존재
		//	 -> 자식에게 공통적인 규약 설정
