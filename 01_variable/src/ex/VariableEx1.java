...
public class VariableEx1 {
	...
	public static void main(String[] args) {
		...
		/* 변수 : 메모리에 값을 저장하는 공간 + 해당 공간에 저장할 수 있는 값을 바꿀 수 있음(변수)
		 * 
		 * Java는 JS와 다르게 처음부터 변수의 자료형을 지정해야 된다
		 * 
		 * 1byte == 8bit
		 * -> 컴퓨터가 데이터 처리(연산)을 위한 최소 단위
		 * 
		 * [Java 기본 자료형 8가지] (암기 필수)
		 * - 논리형
		 * boolean (1btye) - true/false 저장 가능
		 * - 정수형
		 * byte	 (1byte)
		 * short (2byte)
		 * int	 (4byte, 정수 기본형)
		...
		// 변수 선언
		int num1; // 메모리에 int형 정수를 저장할 공간 4byte를 할당하고 해당 공간(변수)의 이름을 num1이라고 지정
				  // int형 정수 : -21.47억 ~ +21.47억
		// Java 지역 변수는 초기화를 하지 않은 경우 출력, 연산을 할 수 없다.(단, 대입 연산은 제외)
		...
		// 2147483648L 또는 2147483648l 표기 == long 인식된다
		long longNum1 = 2147483648L;
		// 자릿수 구분을 위해 _ 사용 가능 -> 연산 시 인식X
		 ...
		 *  - 리터럴 표기법 : ""
		 *  - String은 몇 글자가 작성될지 모르기 때문에 기본적으로 크기가 정해져있는 "기본 자료형"에 포함되지 않는다.
		 *  - 대신 다른 곳에 문자열이 byte[]모양으로 만들어 지고 해당 배열의 주소를 참조하는 "참조형"이 된다
		 */
