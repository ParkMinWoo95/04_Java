...
public class StudentService {
	...
	/* students 객체 배열 요소 중 비어있는(null)
	 * 인덱스를 찾아 전달받은 StudentDTO객체를 추가 후 true 반환
	...
	public boolean addStudent(StudentDTO student) {
		...
		for(int i = 0; i<students.length; i++) {
			if(students[i] == null) { // i 번째 인덱스 요소가 참조하는게 업을 경우
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			...
			// boolean A.equals(B)
			// - A가 참조하는 객체의 필드와 B가 참조하는 객체의 필드가 같은지 비교
			...
			if(std.getName().equals(targetName)) {
				...
				// -> 저장된 이름과 입력된 이름이 같아도 같지 않다(flase)가 발생할 수 있다
	 ...
	 * @param index
	 * @return 1 :범위 초과 / 2 : null / 3 : 참조하는 학생 있음ㄴ
	 */
	...
	public void deleteStudent(int index) {
		...
		// 1) student배열에서 입력 받은 index 번째 요소를 null로 바꿈
		students[index] = null;
